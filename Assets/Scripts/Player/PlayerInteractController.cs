...
public class PlayerInteractController : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (_itemHeavyUI) _itemHeavyUI.SetActive(false);
	}
	...
	private void ShowOutline()
	{
		...
		if (_collectableItem != null)
		{
			if (PlayerCurrentLoad < PlayerMaxLoadCapacity)
				if (_interactUI) _interactUI.SetActive(true);
				else
				{
					if (_itemHeavyUI) _itemHeavyUI.SetActive(true);
				}
		}
	}
	...
	private void HideOutline()
	{
		if (_prevController != null)
		{
			...
			if (_interactUI) _itemHeavyUI.SetActive(false);
			if (_itemHeavyUI) _interactUI.SetActive(false);
		}
	}
}
